...
namespace Konobar
{
    public class Konobar
    {
        ...
        private static void UnosRezervacije()
        {
            ...
            //popuni rezervaciju
            RezervacijeStolova.rezervacije.Add(1, new Dictionary<int, string> { { satRezervacije, imeRezervacije } });
            //postavi sto na rezervisan
            foreach(Sto s in StoloviRepozitorijum.stolovi)
            {
                if(s.brStola == brStola)
                {
                    s.status = StatusSto.REZERVISAN;
                    break;
                }
            }
        }
